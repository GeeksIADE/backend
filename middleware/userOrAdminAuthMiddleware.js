...
async function userOrAdminAuthMiddleware(req, res, next) {
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, user) => {
        ...
        try {
            ...
            if (parseInt(req.params.id) === user.id || user.user_role === 1) {
                req.user = user;
                next();
            } else {
                return res.status(403).json({ message: 'Access Forbidden: Only admins or the user themselves can access this resource' });
            }
        } catch (err) {
